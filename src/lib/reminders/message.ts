...
/**
 * The WhatsApp reminder message text. It is built server-side by
 * `sendFeeReminder`, which logs exactly this text and returns it; the client
 * uses the returned message for the wa.me URL, so the logged message and the
 * actually-sent message can never drift apart.
 *
 ...
 * them), so callers convert via `.toNumber()` before this function runs.
 *
