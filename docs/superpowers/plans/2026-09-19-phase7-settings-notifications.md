...
## Execution Notes: deviations and post-review changes (supersede the code blocks above where they differ)

Task 1 was run first by the orchestrator; Tasks 2-18 were implemented in parallel by dependency-chained agents, committed one task per commit, independently reviewed in four clusters, fixed in two follow-up commits, and reviewed once more as a whole branch. The implementations matched the plan verbatim; the differences below are the plan-level defects review found and the environment facts execution surfaced.

1. **Task 1 migration.** `prisma migrate dev` refuses to run non-interactively because adding the unique `dedupeKey` index raises an interactive warning. The migration was produced with `prisma migrate diff --from-schema-datasource ... --to-schema-datamodel ... --script`, saved as `prisma/migrations/20260919140000_add_settings_and_notification_dedupe/migration.sql`, and applied with `prisma migrate deploy` (additive SQL only; `migrate status` reports up to date). The migration is already applied to the shared database.
2. **Task 4.** The reminder template's minimum length is `.min(1)`, not 10: requiring both `{name}` and `{amount}` already forces at least 14 characters, so a length-10 minimum could never fire. The logo URL scheme is normalised case-insensitively (`HTTPS://` is accepted and stored as `https://`) and capped at 2048 characters.
3. **Task 5.** `buildDueSoonCandidates` normalises `today` to UTC midnight; `paymentModeLabel` no longer returns inherited object members. New pure helper `formatDateTimeIST` in `src/lib/dates.ts` formats real timestamps as an IST date and time (e.g. "20 Sept 2026, 2:00 am"); `formatDateUTC` is only correct for UTC-midnight calendar days and showed notifications/activity a day early between 00:00 and 05:30 IST.
4. **Task 8.** The receipt year is the IST calendar year (`todayInIST().getUTCFullYear()`), not the host-local year.
5. **Task 10.** The low-attendance window is 30 calendar days (`LOW_ATTENDANCE_WINDOW_DAYS - 1` back, inclusive of today), not 31. The claim advances the throttle timestamp before the work runs: a failed sync is retried only after the throttle window, a deliberate backoff that stops a persistently failing sync from running on every page load.
6. **Task 11.** Recent Activity discovers recent attendance roll calls from the latest 60 rows, then re-reads the WHOLE (batch, date) groups (up to 10), so a roll call cut by the 60-row boundary no longer reads "x of 60" (verified: a 62-student roll call reads "40 of 62 present").
7. **Task 15.** The `(app)` layout catches failures loading settings or the unread count (falls back to defaults / 0) so they cannot 500 every page. `createStudent` and `createPayment` also call `revalidatePath("/", "layout")` explicitly so the header badge refreshes without relying on documented-temporary Server Action behaviour (which re-renders from the root). Logo images use `referrerPolicy="no-referrer"` and `loading="lazy"`; long academy names truncate; the bell has a keyboard focus ring.
8. **Task 16/17.** The notifications page uses the true unread total (from `countUnreadNotifications`) for the header text and the Mark All Read button, and notes when it shows only the latest 100; notification and activity times are IST date-times; a failure loading Recent Activity yields an empty feed instead of breaking the dashboard.
9. **Task 13/14.** After a successful save the forms show the normalized values the server stored (trimmed text, uppercased prefix, coerced day counts); the receipt preview appears only for a valid prefix; the Settings page previews with the IST year.
10. **Known and accepted:** old receipts render with the CURRENT academy name/logo/contact/footer (only the number, amount and date are fixed; a future `Receipt.academySnapshot` would preserve the printed header); overdue-cycle windows are aligned to the epoch, so the first re-notification can arrive 1-N days after the first; server actions carry no per-action auth check (the same as every existing action; `src/middleware.ts` gates routes and actions on the session cookie); the dashboard and receipt taglines stay hard-coded.
